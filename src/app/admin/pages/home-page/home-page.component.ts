...
export class HomePageComponent  implements OnInit, OnDestroy{
  ...
  ngOnInit(): void {  
    this.modalService.$modal.subscribe((value)=> {this.modalSwitchCreate = value});
    this.modalService.$modalEdit.subscribe((value)=> {this.modalSwitchEdit = value});
    this.getAllAdvisors();
  ...
  filterAdvisors() {
    this.filteredAdvisors = this.allAdvisors.filter(advisor =>
      advisor.name.toLowerCase().includes(this.searchAdvisor.toLowerCase()) ||
      advisor.lastName.toLowerCase().includes(this.searchAdvisor.toLowerCase())
    );
  ...
  deleteAdvisor(advisorId: any){
    this.alertService.showConfirmationDialog().then((confirmed) => {
      if (confirmed) {
        this.advisorService.deleteAdvisor(advisorId).pipe(
          tap(() => {
            console.log(`asesor con id ${advisorId} eliminado`);
          })
        ).subscribe(
          () => {
            this.alertService.toast('Asesor eliminado correctamente', 'success');
          },
          error => {
